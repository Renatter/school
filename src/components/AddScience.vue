...
export default {
  ...
  methods: {
    subscribeToNews() {
      const newsRef = collection(db, "science");
    ...
    async deleteNews(newsId) {
      ...
      try {
        const docRef = doc(collection(db, "science"), newsId);
    ...
    async addNews() {
      ...
      try {
        const docRef = await addDoc(collection(db, "science"), this.newNews);
